            ...
            return (
              <div key={p.id} className="rounded-2xl border-2 border-emerald-300 bg-emerald-50 dark:bg-emerald-500/10 dark:border-emerald-500/40 p-4 order-enter">
                <Link
                  to="/pedidos/$id"
                  params={{ id: p.id }}
                  className="block -m-4 mb-0 p-4 pb-2 rounded-t-2xl hover:bg-emerald-100/40 dark:hover:bg-emerald-500/15 transition-colors"
                >
                ...
                {!pago && canMarkPago && p.status_financeiro !== "cancelado" && (
                  <button
                    ...
                    className="mt-3 w-full inline-flex items-center justify-center gap-1.5 rounded-lg bg-emerald-500 px-2.5 py-2 text-xs font-bold text-white hover:bg-emerald-600 disabled:opacity-50"
                  >
